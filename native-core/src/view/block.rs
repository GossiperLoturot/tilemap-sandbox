...
impl BlockField {
        ...
        for block in info.blocks {
            ...
            for sprite in block.sprites {
                ...
                sprite_addr.push(ImageAddress {
                    ...
                    atlas_end_index: (images.len() + sprite.images.len()) as u32,
                    ticks_per_image: sprite.ticks_per_image,
    ...
    pub fn update_view(&mut self, dataflow: &dataflow::Dataflow, rect: Rect2) {
        ...
        let rect = IRect2::new(
            dataflow::Dataflow::find_block_page_coord(rect.min),
            dataflow::Dataflow::find_block_page_coord(rect.max),
        );
        ...
        for (page_coord, live_page) in &mut self.live_pages {
            let Ok(page) = dataflow.get_block_page(*page_coord) else {
                continue;
            };
            ...
            let mut count = 0;
            for (i, block) in page.blocks.iter().take(Self::BUFFER_LEN).enumerate() {
